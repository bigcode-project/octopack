...
PUSH_DATASET_NAME = "smallcommits"
...
### FULL ###
PATHS = sorted([os.path.join(BASE_DIR, lang, f) for lang in LANGUAGES for f in os.listdir(BASE_DIR + "/" + lang)])[:10]
print(PATHS)
...
ds = ds.filter(lambda x: (len(x["new_file"].split(".")) > 1) and (x["new_file"].split(".")[-1] == "py"), num_proc=NUM_PROC)

...
commits_file_path = "dataset/methods2test/methods2test_commits.jsonl"
...
def get_file_contents(commit, old_file, new_file,  repo, random_dir, cwd=None):
...
def get_diff(ex):
    ...
    with open('dataset/methods2test/methods2test.jsonl', 'a') as methods2test_file:  # Open the file in append mode
        ...
        try:
            ...
            logging.error(f'ERROR: No files found for commit {commit_id} , error: {e}')
        ...
        if completed.returncode != 0:
            ...
            for file in files:
                ...
                if file.endswith(".java"):
                    ...
                    new_file = file
                    old_file = file
                    new_contents, old_contents ,_ , _ =  get_file_contents(commit_id, old_file, new_file, repo, random_dir,  working_dir)
                    #get commit message
...
if __name__ == "__main__":
    ...
    START = 35 # Modify for each instance (0 - 7)
    samples_per_instance =  1 * 4 * 5 * 5    # 1 * 4 * 64 * 34 # 8_388_608
